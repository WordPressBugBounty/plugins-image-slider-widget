...
function ewic_free_plugin_page() {
	...
	$plugins_allowedtags = array(
		'a' => array( 'href' => array(), 'title' => array(), 'target' => array(), 'aria-label' => array(), 'class' => array() ),
		'abbr' => array( 'title' => array() ), 'acronym' => array( 'title' => array() ),
		'code' => array(), 'pre' => array(), 'em' => array(), 'strong' => array(),
		'div' => array( 'class' => array() ), 'span' => array( 'class' => array() ),
		'p' => array(), 'ul' => array(), 'ol' => array(), 'li' => array(), 'cite' => array(),
...
.ewic-container-cnt .plugin-card-top .column-description {
	margin-left: 140px !important;
}

body.rtl .ewic-container-cnt .plugin-card-top .column-description {
...
<div style="margin-top:30px;" class="wp-list-table widefat plugin-install">
	<div id="the-list">
		...
		foreach ( (array) $item as $plugin ) {
				...
				switch ( $status['status'] ) {
					case 'install':
						if ( $status['url'] ) {
							...
							$action_links[] = '<a class="install-now button-secondary ewic-button-install" href="' . esc_url( $status['url'] ) . '" aria-label="' . esc_attr( sprintf( __( 'Install %s now' ), $name ) ) . '">' . __( 'Install Now' ) . '</a>';
						}
					...
					case 'update_available':
						if ( $status['url'] ) {
							/* translators: 1: Plugin name and version */
							$action_links[] = '<a class="button ewic-button-update" href="' . esc_url( $status['url'] ) . '" aria-label="' . esc_attr( sprintf( __( 'Update %s now' ), $name ) ) . '">' . __( 'Update Now' ) . '</a>';
						}
			...
			$details_link   = self_admin_url( 'plugin-install.php?tab=plugin-information&amp;plugin=' . esc_attr( $plugin['slug'] ) .
		...
		<div id="<?php echo esc_attr( $plugin["slug"] ); ?>" class="plugin-card drop-shadow lifted">
            ...
            <?php if ( isset( $plugin["slug"] ) && $plugin["slug"] == 'easy-media-gallery' ) {echo '<div class="most_popular"></div>';} ?>
				<a href="<?php echo esc_url( $details_link ); ?>" class="ewic-free-plgn-icon thickbox plugin-icon"><img width="128" height="128" src="<?php echo esc_attr( $plugin_icon_url ) ?>" /></a>
				...
				<div class="desc column-description" style="margin-right: 20px !important;">
					...
					<?php
						if ( $action_links ) {
							...
							echo '<li>' . wp_kses( $action_links[0], $plugins_allowedtags ) . '</li>';
